...
- **Dueños de Gimnasios**: Registro con periodo de prueba de 30 días, consulta de cuentas oficiales VersaGym para realizar transferencias, gestión de la suscripción SaaS, copia de la clave de licencia y consulta de respaldos remotos.
...
1. **Vistas y Páginas (`src/pages/`)**:
   ...
   - `dashboard/Overview.tsx`: Resumen de estado de licencia, contador de días restantes y botón de copia rápida de la `license_key`.
   - `dashboard/Subscription.tsx`: Catálogo de planes SaaS, carga dinámica de cuentas oficiales VersaGym y modal para reportar comprobantes de membresía.
...
3. **Manejo de Peticiones y Caché (`src/api/` y `src/hooks/`)**:
   ...
   - `useAuth.ts`: Custom hook que envuelve las mutaciones de Login, Registro y Logout usando `@tanstack/react-query`.
   - `useSubscription.ts`: Custom hook que gestiona la carga de planes, estado de la suscripción e historial de pagos reportados.
...
| `subscriptionApi.getCurrentSubscription` | `GET /api/subscription` | Obtiene el estado y licencia de la suscripción. |
...
| `paymentApi.getPayments` | `GET /api/payments` | Recupera el historial de pagos de suscripción del gimnasio. |
